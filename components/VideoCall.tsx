...
const VideoCall: React.FC<VideoCallProps> = ({ roomId, userId }) => {
  ...
  const [isLocalVideoExpanded, setIsLocalVideoExpanded] = useState<boolean>(false)
  ...
  }, [isMediaSupported, localStream]) // Added localStream to dependencies
    ...
    const subscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          let signal: Signal
          try {
            signal = typeof payload.new.payload === "string" ? JSON.parse(payload.new.payload) : payload.new.payload
          } catch (parseError) {
  ...
  const toggleLocalVideoExpansion = () => {
    setIsLocalVideoExpanded(!isLocalVideoExpanded)
  }

  return (
    <div className="video-call-container h-screen w-screen relative overflow-hidden">
      {error && (
        <div className="error-message absolute top-0 left-0 right-0 bg-red-100 border border-red-400 text-red-700 px-4 py-3 z-50">
          {error}
        </div>
      )}
      {!isMediaSupported ? (
        <div className="absolute top-0 left-0 right-0 bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 z-50">
          Your browser does not support video calls. Please use a modern browser like Chrome, Firefox, or Safari.
        </div>
      ) : (
        <>
          <div className="connection-status absolute top-4 left-4 bg-black bg-opacity-50 text-white px-2 py-1 rounded z-40">
            {connectionState || "Not connected"}
            {isCaller && <span className="ml-2">(Caller)</span>}
          </div>
          <div className="remote-video absolute inset-0">
            <video ref={remoteVideoRef} autoPlay playsInline className="w-full h-full object-cover" />
          </div>
          <div
            className={`local-video absolute z-10 transition-all duration-300 ease-in-out cursor-pointer ${
              isLocalVideoExpanded
                ? "bottom-4 right-4 w-1/3 h-1/3 max-w-md max-h-md"
                : "bottom-4 right-4 w-1/4 h-1/4 max-w-xs max-h-xs"
            }`}
            onClick={toggleLocalVideoExpansion}
          >
